<?php
@$zamestnanec=base64_decode(@$_GET["zamestnanec"]);
@$jmeno=@$_GET["jmeno"];
...
$pdf->Write(12,"Tisk Docházky Zamìstnance: ".@$zamestnanec." za Období: ".$obdobi1[1].".".$obdobi1[0]."                                                                                               Tisk Dne: ".date("d.n.Y"));$pdf->Ln(6);
$pdf->Write(7,$jmeno);$pdf->Ln();
...
$pdf->Cell(191,0.4,"",'T');
$pdf->Ln();
$pdf->Cell(20,6,"Datum",'LB');
$pdf->Cell(25,6,"Pøíchod",'LBR');
$pdf->Cell(25,6,"Odchod",'LBR');
$pdf->Cell(24,6,"Celk.Èas.Def.",'LBR');
$pdf->Cell(36,6,"Definováno",'LBR');
$pdf->Cell(61,6,"Poznámka",'LBR');
$pdf->Ln();
$pdf->Cell(191,0.4,"",'T');
$pdf->Ln();
...
@$vysledek = mysql_query("select * from dochazka where osobni_cislo='$zamestnanec' and obdobi='$obdobi' order by cas,datum,id");
...
if (mysql_result($vysledek,@$vypis,4)==$prichod and mysql_result($vysledek,@$vypis,2)==$datum) {$in =$in.substr(mysql_result($vysledek,@$vypis,3),0,5).",";}
@$vypis++;endwhile;
...
if ( $odchody=="YES" and mysql_result($vysledek,@$vypis,2)==$datum) {$out =$out.substr(mysql_result($vysledek,@$vypis,3),0,5).",";}
@$vypis++;endwhile;
