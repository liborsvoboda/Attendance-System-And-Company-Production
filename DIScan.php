...
$reload=@$_GET["reload"];if (!$reload){$reload=1;}if (!mysql_num_rows(mysql_query("select id from infopanel where strana='$reload'"))) {$reload=1;}
@$text=@$_GET["text"];
@$pokus=@$_GET["pokus"];
...
<script language="JavaScript">
function infopanel ( ){  var panelTime = new Date ( );
  var infostarth=panelTime.getHours ( );
  var infostartm=panelTime.getMinutes ( );
  var infostarts=panelTime.getSeconds ( );
<?$timelimit=mysql_result(mysql_query("select hodnota from setting where nazev='Infočas'"),0,0);@$tmin=floor(@$timelimit/60);@$tsec=$timelimit-(@$tmin*60);?>
  var starts = <?if ((StrFTime("%S", Time())+$tsec)<60) {echo (StrFTime("%S", Time())+$tsec);} else {echo ((StrFTime("%S", Time())+$tsec)-60);$tmin=$tmin+1;}?>;
  var startm = <?if ((StrFTime("%M", Time())+$tmin)<60) {echo (StrFTime("%M", Time())+$tmin);$thour=0;} else {echo ((StrFTime("%M", Time())+$tmin)-60);$thour=1;}?>;
  var starth = <?if ((StrFTime("%H", Time())+$thour)<24) {echo (StrFTime("%H", Time())+$thour);} else {echo "0";}?>;
  var clanek= "<? echo $idpanel=mysql_result(mysql_query("select strana from infopanel where strana='$reload'"),0,0);$screentime=mysql_result(mysql_query("select cas from infopanel where strana='$reload'"),0,0);?>";
  var reload= "<?echo @$_GET["reload"];?>";

if (((infostarts==starts && infostartm==startm && infostarth==starth) || reload!="") && clanek!=null) {document.write('<html><head><title>Docházkový Systém</title><meta http-equiv=Content-Type content=text/html;charset=windows-1250><script>setTimeout(\'window.location.href=\"DIScan.php?reload=<?echo ($idpanel+1);?>\"\', <?echo $screentime;?>000)<\/script></head><body onclick=\"window.location.assign(\'DIScan.php\')\" ><div style=width:100%;height:100%><?echo mysql_real_escape_string(mysql_result(mysql_query("select obsah from infopanel where strana='$reload' "),0,0));?></div></body></html>');}
...
<body onload="updateClock();setInterval('updateClock()', 1000 );infopanel();setInterval('infopanel()', 1000 );" bgcolor="#E6AD6F" style=margin:0;overflow:hidden;>
...
if (mysql_result($data100,0,0)<>""){echo "<tr><td width=100%><input type=button value='".mysql_result($data100,0,0)."' style=width:100%;height:70px;font-size:45px;";?> onclick="window.location.href=('DIScan.php?numlock=<?echo base64_encode(mysql_result($data100,0,2));?>&text=<?echo mysql_result($data100,0,0);?>')" <?echo" ></td></tr>";}
...
if (oscislo==null) {window.location.href="DIScan.php?pokus=false&oscislo="+oscislo;}
if (oscislo.length> 12) {window.location.href="DIScan.php?pokus=false&oscislo="+oscislo;}
if (oscislo.length!=12 && oscislo.length< 12) {window.location.href="DIScan.php?pokus=false&oscislo="+oscislo;}
if ("<?echo $oscisla?>".search(","+oscislo+",")<0 && "<?echo $text?>" != "Žádost o Aktivaci" && oscislo.length==12) {window.location.href="DIScan.php?numlock=<?echo base64_encode(@$numlock);?>&text=<?echo @$text;?>&cyklus=<?echo base64_encode(@$cyklus);?>&oscislo="+oscislo;}
if ("<?echo $oscisla?>".search(","+oscislo+",")>=0 && "<?echo $text?>" != "Žádost o Aktivaci" && oscislo.length==12) {window.location.href="DIScan.php?numlock=<?echo base64_encode(@$numlock);?>&text=<?echo @$text;?>&cyklus=<?echo base64_encode(@$cyklus);?>&oscislo="+oscislo;}
if ("<?echo $text?>"=="Žádost o Aktivaci" && oscislo!="null" && oscislo.length==12) {window.location.href="DIScan.php?numlock=<?echo base64_encode(@$numlock);?>&text=<?echo @$text;?>&cyklus=<?echo base64_encode(@$cyklus);?>&oscislo="+oscislo;}
<?}?>
<?if (mysql_result(mysql_query("select hodnota from setting where nazev='Snímač' "),0,0)=="Linux") {?>
if (oscislo==null) {window.location.href="DIScan.php?pokus=false";}
if (oscislo.length< 20) {window.location.href="DIScan.php?pokus=false";}
if ("<?echo $text?>" != "Žádost o Aktivaci" && oscislo!="null") {window.location.href="DIScan.php?numlock=<?echo base64_encode(@$numlock);?>&text=<?echo @$text;?>&cyklus=<?echo base64_encode(@$cyklus);?>&oscislo="+oscislo;}
if ("<?echo $text?>"=="Žádost o Aktivaci" && oscislo!="null") {window.location.href="DIScan.php?numlock=<?echo base64_encode(@$numlock);?>&text=<?echo @$text;?>&cyklus=<?echo base64_encode(@$cyklus);?>&oscislo="+oscislo;}
<?}?>
</SCRIPT>
<script language="JavaScript">setTimeout('window.location.href="DIScan.php"', 1000)</script>
<?}
...
<script language="JavaScript">setTimeout('window.location.href="DIScan.php"', 30000)</script>
<?}
...
<script language="JavaScript">setTimeout('window.location.href="DIScan.php"', 2000)</script>
<?}
...
<script language="JavaScript">setTimeout('window.location.href="DIScan.php"', 3000)</script>
<?}
...
<script language="JavaScript">setTimeout('window.location.href="DIScan.php"', 4000)</script>
<?}
